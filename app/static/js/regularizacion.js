...
(function(){
  ...
  function parseStock(rows){
    ...
    const c = {code:t.col('Código'), name:t.col('Nombre'), stock:t.col('Stock físico'), estado:t.col('Estado del conteo'),
      ...
      um:t.col('Unidad', 'Unidad QMS', 'Unidad de medida', 'U. Medida', 'UM', 'Unidad Defontana')};
    const out = [];
    for (const r of t.rows){
      ...
      out.push({code, key:keyOf(code), name:r[c.name] ?? '', stock:num(r[c.stock]), estado:c.estado >= 0 ? r[c.estado] ?? '' : '',
        por:c.por >= 0 ? r[c.por] ?? '' : '', fecha:c.fecha >= 0 ? parseDate(r[c.fecha]) : null, um:c.um >= 0 ? String(r[c.um] ?? '').trim() : ''});
    }
    return out;
  }
  ...
  const UM_GRUPOS = {
    ...
    unidad:['UN','UND','UNI','UD','UNIDAD','UNIDADES','CU'], pack:['PK','PACK','PACKS'], rollo:['RL','ROLLO','ROLLOS'], caja:['CJ','CAJA','CAJAS']
  };
  const UM_NOMBRE = {metro:'metros', pie:'pies', pulgada:'pulgadas', centimetro:'centímetros', milimetro:'milímetros', litro:'litros', galon:'galones', kilo:'kilos', libra:'libras', gramo:'gramos', unidad:'unidades', pack:'packs', rollo:'rollos', caja:'cajas'};
  const UM_POR_FORMA = {};
  ...
  function conteoDelSistema(){
    ...
    return data.map(([code, name, stock, estado, por, fecha, um]) => ({code, key:keyOf(code), name, stock, estado, por, fecha:parseDate(fecha), um: um || ''}));
  }
  ...
  function compute(movList){
    ...
    const mergeGroup = list => {
      ...
      const g = {code:main.code, key:main.key, name:main.name, por:main.por, estado:main.estado, um:main.um || '',
    ...
    const build = (s, docs) => {
      ...
      return {cause:dx.cause, obs:dx.obs, umRound: !!dx.umRound, alias, viaName, nCounted: s ? s.nCounted : 0, key, code: s ? s.code : docs[0].art, name: s ? s.name : docs[0].desc, s, umInfo, counted, docs, ins, outs, sameDay, aj, ajVal, ajDocs, sysCalc, toOther,
  ...
  function renderPlan(){
    ...
    box.innerHTML = `
      ...
      ${sec(3, 'Entradas (Parte de Entrada)', 'Una Parte de Entrada por ajuste de inventario con estas líneas.', P.in,
        '<th>Producto</th><th class="num">Debería tener hoy</th><th class="num">Tiene Defontana hoy</th><th class="num">Cantidad</th><th class="num">Costo c/u</th><th class="num">Total</th><th>Motivo</th>',
        ...
        `<td></td><td colspan="5"><b>Total</b></td><td class="num"><b>${money(tot(P.in))}</b></td><td></td>`)}
      ${sec(4, 'Salidas (Parte de Salida)', 'Una Parte de Salida por ajuste de inventario / merma con estas líneas. Defontana pondrá el PMP del día; el valor es referencial.', P.out,
        '<th>Producto</th><th class="num">Debería tener hoy</th><th class="num">Tiene Defontana hoy</th><th class="num">Cantidad</th><th class="num">PMP c/u</th><th class="num">Total</th><th>Motivo</th>',
        ...
        `<td></td><td colspan="5"><b>Total</b></td><td class="num"><b>${money(tot(P.out))}</b></td><td></td>`)}
  ...
  function planExcel(){
    ...
    hoja('3 Entradas', P.in.map((x, i) => ({'#': i + 1, 'Código': x.r.code, 'Nombre': x.r.name, 'Debería tener hoy': x.r.realNow ?? '', 'Tiene Defontana hoy': hoyDe(x.r) ?? '', 'Cantidad': x.qty, 'Costo c/u': r2(x.v), 'Total': x.v != null ? Math.round(x.v * x.qty) : '', 'Origen del costo': x.src, 'Motivo': x.motivo})));
    hoja('4 Salidas', P.out.map((x, i) => ({'#': i + 1, 'Código': x.r.code, 'Nombre': x.r.name, 'Debería tener hoy': x.r.realNow ?? '', 'Tiene Defontana hoy': hoyDe(x.r) ?? '', 'Cantidad': x.qty, 'PMP c/u (referencial)': r2(x.v), 'Total': x.v != null ? Math.round(x.v * x.qty) : '', 'Motivo': x.motivo})));
